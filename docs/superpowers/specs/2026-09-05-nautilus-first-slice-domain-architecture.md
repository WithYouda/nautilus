...
以下是对现有契约的最小纠偏，不把仍待作者决定的产品取舍写成已确认结论：

- **首次授权**：未认证、网络可达的接口不得直接返回可用于建立用户会话的实际授权秘密。当前支持到何种网络信任范围仍是产品决定；修复不得以破坏必要的 WSL2 宿主访问为代价。
- **彻底删除与恢复**：受控恢复必须保留当前已知的 purge/tombstone 屏障。恢复前只扫描候选备份是否直接包含当前已删除正文并不充分；若一次恢复会丢失删除标记、使后续更早备份可能复活内容，应拒绝恢复。历史 schema v1 私文和全副本清除仍按未完成边界处理。
- **首页降级**：学习状态、返回卡、创建和继续学习是核心路径；测量/指标读取失败只能让指标区域降级，不能让核心路径整体不可用。
- **返回解释**：返回服务已经生成的规则推荐理由属于核心承诺的一部分，前端必须实际呈现；不因此引入新的 AI 推荐器或完整成果图。

上述条目目前是审查结论和后续验收边界，不表示代码已经修复或本次自动化验证已经重新运行。
...
- 彻底删除必须显示受影响的主张、状态和回访安排并明确确认；内容不可恢复或不可逆脱敏，缓存、索引和普通备份不得继续提供内容，只保留最小审计元数据。受控恢复必须同时证明不会丢失当前已知的删除屏障；不能证明时安全拒绝，不允许通过先恢复到缺少 tombstone 的旧状态再绕过后续复活检查。
...
7. 涉及首次授权、彻底删除或恢复路径的变更必须有对应的负向用例；诊断指标故障不得阻断首页核心学习路径，返回卡的规则推荐理由必须在用户界面可见。
