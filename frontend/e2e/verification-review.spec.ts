...
test('saved answers → per-question discussion with local sources → completed history → refresh → permanent deletion on mobile', async ({ page }) => {
  ...
  await expect(discussion.getByRole('button', { name: '取消生成' })).toBeHidden();
  await expect(discussion.locator('.ai-message--user')).toHaveCount(2);
  await expect(discussion.getByLabel('回答 2/2', {exact:true})).toBeVisible();
  await discussion.getByRole('button', {name:'上一个回答',exact:true}).click();
  await expect(answer.last()).toHaveText('合成续学讲解：');
  await expect(discussion.getByLabel('回答 1/2', {exact:true})).toBeVisible();
  await page.reload();
  await expect(discussion.getByLabel('回答 1/2', {exact:true})).toBeVisible();
