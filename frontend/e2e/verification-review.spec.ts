...
test('saved answers → per-question discussion with local sources → completed history → refresh → permanent deletion on mobile', async ({ page }) => {
  ...
  await page.setViewportSize({ width: 390, height: 844 });
  await input.fill('保留在底部的讨论草稿');
  const originalQuestion = discussion.locator('.ai-message--user').first();
  await originalQuestion.getByRole('button', { name: '修改', exact: true }).click();
  const editForm = originalQuestion.getByRole('form', { name: '修改消息' });
  const editInput = editForm.getByRole('textbox', { name: '修改消息内容' });
  await editInput.fill('合成修改追问：先单独检查空输入');
  await expect(editForm.getByRole('button', { name: '取消', exact: true })).toBeVisible();
  await expect(editForm.getByRole('button', { name: '发送', exact: true })).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBeTruthy();
  ...
  await expect(originalQuestion).toContainText('合成修改追问：先单独检查空输入');
  await expect(discussion.locator('.ai-message--assistant').first()).toContainText('合成续学讲解：可以继续分析不同输入');
  ...
  const secondQuestionInDiscussion = discussion.locator('.ai-message--user').last();
  await secondQuestionInDiscussion.getByRole('button', { name: '修改', exact: true }).click();
  const secondEdit = secondQuestionInDiscussion.getByRole('form', { name: '修改消息' });
  await secondEdit.getByRole('textbox', { name: '修改消息内容' }).fill('合成修改取消后追问');
  await secondEdit.getByRole('button', { name: '发送', exact: true }).click();
  await expect(secondQuestionInDiscussion).toContainText('合成修改取消后追问');
  await expect(answer.last()).toContainText('合成续学讲解：可以继续分析不同输入');
  await secondQuestionInDiscussion.getByRole('button', { name: '上一个消息' }).click();
  await expect(secondQuestionInDiscussion).toContainText('合成追问：验证取消后重试。');
  await expect(answer.last()).toHaveText('合成续学讲解：');
  await expect(discussion.getByLabel('回答 1/2', {exact:true})).toBeVisible();
  await page.reload();
  await expect(discussion.getByLabel('回答 1/2', {exact:true})).toBeVisible();
